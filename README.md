...
- ✅ メンション・リプライに対して会話応答
- ✅ DM でもそのまま会話可能
- ✅ 本人履歴、相手のプロフィール、サーバー/ワールドの正式プロフィールを状況に応じて自動選択
- ✅ `ollama.embed()` を使った semantic memory により、意味的に近い過去発言も参照可能
- ✅ README と `knowledge/chat_rag.md`、`data/channel_rag/<guild_id>/chat_rag.md` をローカル知識として参照可能
- ✅ リモート接続 + API キー構成では必要時のみ web search / web fetch を使用し、検索した URL を返答とログに残す
...
- ✅ `data/channel_rag/<guild_id>/` にサーバー/チャンネル固有の設定、ログ、説明文を集約可能
...
- `meeting.audio_max_total_mb` / `meeting.audio_max_user_mb`: 録音取り込み上限。`0` で無制限
...
- ✅ 明示 @mention がある場合は対象ユーザーの発言とプロフィールを優先
- ✅ `getplayerinfo` / `getserverinfo` 相当の一括プロフィールを内部で参照
- ✅ semantic memory で意味的に近い過去発言も補助的に参照
- ✅ 時刻付き履歴を含める（「xx時にこんなこと言ってた」を記憶）
- ✅ 同じサーバー内でユーザー ID で個人を識別
- ✅ README や `knowledge/chat_rag.md`、`data/channel_rag/<guild_id>/chat_rag.md` の内容を参照して Bot 自身の仕様説明に回答可能
- ✅ チャンネル固有の説明文を `data/channel_rag/<guild_id>/chat_rag.md` に蓄積して会話応答へ反映可能
- ✅ `data/channel_rag/<guild_id>/rules.md` / `settings.yaml` / `logs/` / `channels/<channel_id>/` を分けて管理可能
...
Bot 固有の説明、サーバー運用メモを別ファイルで持たせたい場合は次を使えます。

- `knowledge/chat_rag.md`
- `data/channel_rag/<guild_id>/chat_rag.md`
- `data/channel_rag/<guild_id>/rules.md`
- `data/channel_rag/<guild_id>/settings.yaml`
- `runtime/logs/channel_rag/<guild_id>/logs/messages.log`
- `runtime/logs/channel_rag/<guild_id>/channels/<channel_id>/logs/messages.log`
- `runtime/logs/message_logs/guild_<guild_id>_channel_<channel_id>.json`

まずは `knowledge/chat_rag.md` を使うのが一番簡単です。README と同様に会話中のローカル知識として参照されます。
サーバー全体の説明や運用ルールは `data/channel_rag/<guild_id>/chat_rag.md` と `rules.md`、チャンネル別のメモは `data/channel_rag/<guild_id>/channels/<channel_id>/` が向いています。
...
この移行はファイル構造をそのままコピーします。実際の guild ID / channel ID に合わせた整理は、必要なら `data/channel_rag/<guild_id>/` を手で調整してください。
...
これは `runtime/logs/message_logs/*.json` を読み、`ollama.model_embedding` で batch embedding を作って `runtime/rag/message_vectors.sqlite3` に保存します。
既存の `data/message_logs/*.json`、`runtime/history/message_logs/*.json`、`data/channel_rag/<guild_id>/channels/<channel_id>/messages.json` も互換入力として読みます。
...
- **互換入力**: 既存の `data/message_logs/*.json` / `runtime/history/message_logs/*.json` / `data/channel_rag/<guild_id>/channels/<channel_id>/messages.json`
- **新規保存**: `runtime/logs/message_logs/*.json`
...
./.venv/bin/python bin/debug_route.py mention --preset web_search --mock-llm
```

`--mock-llm` を付けると、外部 AI / Web 検索なしでメンションの実フローを通せます。
