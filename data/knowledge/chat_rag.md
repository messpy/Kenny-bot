...
## 説明ルール
Bot 自身の機能やコマンドの使い方を説明するときは、`src/kennybot/utils/command_catalog.py` の確認済み情報と、このファイルに明記された運用メモを優先する。
README は利用者向け概要であり、会話用 RAG の一次ソースとして扱わない。
このファイルは補助メモなので、command_catalog にないコマンド名や引数を作って補完しない。
