...
class OllamaClientFallbackTest(TestCase):
    def test_all_gemini_models_keep_explicit_ollama_fallback(self) -> None:
        ...
        with patch(
            ...
            candidates = service._candidate_models("gemini-3.5-flash")

        self.assertEqual(candidates, ["gemini-3.5-flash", "gpt-oss:120b-cloud"])
    ...
    def test_gateway_provider_defaults_to_gemini_for_gemini_model(self) -> None:
        ...
        with patch.dict("os.environ", {}, clear=True):
            self.assertEqual(service._ai_gateway_provider("gemini-3.5-flash"), "gemini")
    ...
    def test_chat_simple_uses_ai_gateway_when_configured(self) -> None:
        ...
        with patch.dict("os.environ", {"AI_GATEWAY_URL": "http://gateway.test"}, clear=True):
            result = service.chat_simple("gemini-3.5-flash", "hello")
